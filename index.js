...
app.get('/', async (req, res) => {
    ...
    try {
        ...
        if (query) {
            ...
            conteudoHtml = `<h2>🎬 Filmes Populares</h2><div class="row">${filmes.data.results.map(f => `<div class="card" onclick="abrirPlayer('movie', '${f.id}')"><img src="https://image.tmdb.org/t/p/w300${f.poster_path}"></div>`).join('')}</div><h2>📺 Séries</h2><div class="row">${series.data.results.map(s => `<div class="card" onclick="abrirPlayer('tv', '${s.id}')"><img src="https://image.tmdb.org/t/p/w300${s.poster_path}"></div>`).join('')}</div><h2>🎌 Animes</h2><div class="row">${animes.data.results.map(a => `<div class="card" onclick="abrirPlayer('tv', '${a.id}')"><img src="https://image.tmdb.org/t/p/w300${a.poster_path}"></div>`).join('')}</div>`;
        }
...
<head>
    ...
    <style>
        ...
        input { padding: 8px; border-radius: 5px; border: none; width: 60%; }
        .btn-busca { background: #222; color: #fff; border: none; padding: 8px 15px; border-radius: 5px; font-weight: bold; }
        h2 { margin: 20px 15px 10px; text-align: left; font-size: 18px; }
        ...
        .btn-fechar { background: #e50914; color: #fff; border: none; padding: 10px 15px; border-radius: 5px; font-weight: bold; }
        iframe { width: 100%; height: calc(100% - 60px); border: none; }
    </style>
</head>
<body>
    <header>
        <div style="font-size: 22px; font-weight: bold;" onclick="window.location.href='/'">MAXFLIX</div>
    ...
    <script>
        // Bloqueador de Pop-ups agressivo
        window.open = function() { return null; }; 

        function abrirPlayer(tipo, id) {
            const modal = document.getElementById('player-modal');
            const frame = document.getElementById('video-iframe');
            // Usando o vidsrc.xyz com ID do TMDB
            frame.src = "https://vidsrc.xyz/embed/" + tipo + "/" + id;
